...
![image](https://github.com/Namkwangwoon/TIL-Algorithm-/assets/19163372/f92235c0-4465-464a-8a0d-218ad2c9c78f)
...
![image](https://github.com/Namkwangwoon/TIL-Algorithm-/assets/19163372/efc9eb26-ef89-46ca-bc22-823d8db5cc63)

그 후, 판매원 john 에 의하여 400 원의 이익이 발생합니다. john 은 10% 인 40 원을 센터에 배분하고 자신이 나머지인 360 원을 가집니다. 이 상태를 그림으로 나타내면 아래와 같습니다.

![image](https://github.com/Namkwangwoon/TIL-Algorithm-/assets/19163372/d1e3b6b4-2693-4131-a80c-d7cc0b113f30)
...
![image](https://github.com/Namkwangwoon/TIL-Algorithm-/assets/19163372/396a89a0-f26e-4da0-8d3f-4693b442d930)
...
![image](https://github.com/Namkwangwoon/TIL-Algorithm-/assets/19163372/e3e6a81c-ede6-49a1-840d-3a186558de8c)

마지막으로, 판매원 mary 는 1,000 원의 이익을 달성하고, 이 중 10% 인 100 원을 센터에 배분한 후 그 나머지인 900 원을 자신이 가집니다. 이 상태를 그림으로 나타내면 아래와 같습니다.

![image](https://github.com/Namkwangwoon/TIL-Algorithm-/assets/19163372/15fd6634-d96b-4178-bb8b-fc29a2fbfd8c)

위와 같이 하여 모든 조직 구성원들의 이익 달성 현황 집계가 끝났습니다. 지금까지 얻은 이익을 모두 합한 결과를 그림으로 나타내면 아래와 같습니다.

![image](https://github.com/Namkwangwoon/TIL-Algorithm-/assets/19163372/0c0db995-0aba-4feb-9485-2f7de17cf54b)
...
def solution(enroll, referral, seller, amount):
    ...
    for s, a in zip(seller, amount):
        ...
        cur_seller = s
        while True:
            next_price = cur_price//10
            cur_price = cur_price-next_price
            result[cur_seller] += cur_price
            if graph[cur_seller]=='-' or next_price==0: break
            cur_price = next_price
            cur_seller = graph[cur_seller]
...
테스트 4 〉	통과 (0.21ms, 10.1MB)
테스트 5 〉	통과 (1.67ms, 10.4MB)
테스트 6 〉	통과 (1.92ms, 12.6MB)
테스트 7 〉	통과 (2.24ms, 12.7MB)
테스트 8 〉	통과 (3.09ms, 12.8MB)
테스트 9 〉	통과 (13.83ms, 13.9MB)
테스트 10 〉	통과 (152.98ms, 20.9MB)
테스트 11 〉	통과 (105.68ms, 20.4MB)
테스트 12 〉	통과 (150.51ms, 20.6MB)
테스트 13 〉	통과 (120.69ms, 20.4MB)
```
- 전략
  - 누군가가 이익을 얻었을 때, 자신의 추천인만 잘 찾아가면 된다.
  - 각 추천인을 dictionary에 저장하고 이익이 발생할 때 마다 10%를 넘겨줌
  - 자신의 이익의 10%가 1원 미만이면, 나누지 않고 자신이 100% 가짐 (이익이 10원보다 작을 때)
  - 문제를 잘못 이해해서 약간의 시간이 걸렸다.
    - 문제를 잘 읽자! 
